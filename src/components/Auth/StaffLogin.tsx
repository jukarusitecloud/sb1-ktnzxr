...
export default function StaffLogin({ onResetPassword, onBack }: StaffLoginProps) {
  ...
  const handle2FAVerify = async (code: string) => {
    ...
    try {
      setError(null);
      await loginAsStaff(credentials.email, credentials.password, code);
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <Users className="h-6 w-6 text-mono-900" />
        <h2 className="text-2xl font-bold text-mono-900">スタッフログイン</h2>
      </div>

      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-blue-800">
        <h3 className="font-medium mb-2">デモ用アカウント:</h3>
        <div className="space-y-1 text-sm">
          <p>メールアドレス: staff1@example.com</p>
      ...
      >
        {isSubmitting ? 'ログイン中...' : 'スタッフとしてログイン'}
      </button>
    </form>
  );
}